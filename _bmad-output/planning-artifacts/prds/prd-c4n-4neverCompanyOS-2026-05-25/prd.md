...
9. **OQ-N: Vault directory layout spec.** Mentioned in FR-3, FR-29, FR-30. The build plan flags the vault directory layout as a follow-up artifact ("Should be drafted in M1 since it shapes everything downstream") but does not name an owner or deliverable shape. **Owner:** PM + Architect (M1). **Revisit:** before M1 stories are written.
...
13. **OQ-L (Tier 3 deferred):** team size and budget envelope.
14. **OQ-M (Tier 4 deferred):** attribution copy and contribution-back policy.
...
**Total estimated duration:** ~24–32 weeks (≈6–7 months) per build plan, assuming 2–3 engineers (OQ-L deferred).
