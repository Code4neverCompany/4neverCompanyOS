...
| 1.16  | Hermes TUI embedded as a pane              | 🟡 In progress  | 1.16a ✅ 823b5ff                | **1.16a done** — supervisor PTY upgrade (portable-pty 0.9, `.pty.raw` tap file, JSONL preserved, stdout forward). 10 tests passing + 1 ignored (Windows ConPTY edge case). **Next: 1.16b** — `spawn_hermes` Tauri command. **Then 1.16c** — xterm.js frontend + tail bridge. |
