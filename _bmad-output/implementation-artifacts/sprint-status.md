...
| 1.16  | Hermes TUI embedded as a pane              | 🟡 In progress  | 1.16a 823b5ff · 1.16b 24645b9   | **1.16a + 1.16b done** — backend complete. Hermes spawns into its own Zellij session via the PTY-wrapped supervisor; `.pty.raw` tap files ready for xterm.js consumption. 15 tests passing + 1 ignored. **1.16c queued** — xterm.js frontend + tail bridge (~380 LOC). |
