...
M0 is fully shipped. M1's first batch (1.10–1.14) is shipped, plus 1.15 audit-trail and the entire 1.16a/b/c backend + display layer for Hermes TUI embedding. Story 1.16d (bidirectional input via `.pty.in`) is queued; Stories 1.17 (NSIS installer), 1.18 (E2E), 1.19 (attribution surfaces) close M1.
...
| 1.16  | Hermes TUI embedded as a pane              | 🟡 In progress  | 1.16a 823b5ff · 1.16b 24645b9 · 1.16c (this commit) | **1.16a + 1.16b + 1.16c done** — backend + read-only display complete. `tail_persona_pty` streams `.pty.raw` bytes via Tauri Channel; PtyTail wraps xterm.js; MemoryView (replaces placeholder) hosts Hermes terminal; ProjectsView embeds Dev terminal in running state. 19 tests pass + 1 ignored. **1.16d queued** — bidirectional input via `.pty.in` (~150 LOC). |
| 1.16d | Hermes TUI — bidirectional input           | ⏸ Pending       | —                               | Split from 1.16c. Supervisor `.pty.in` watcher + `write_persona_pty_in` Tauri command + PtyTail.onData → write wiring. ~150 LOC.                                                                                                                                       |
...
This sprint: ✅ **Story 1.16c done.** Next is **Story 1.16d** — bidirectional input via `.pty.in` (supervisor watcher + `write_persona_pty_in` command + PtyTail.onData wiring). After 1.16d, the remaining M1 stories are 1.17 (NSIS installer), 1.18 (E2E test), 1.19 (attribution surfaces).
