...
- **Smoke-test**: result of the smoke-test on the reference Windows machine _(filled in during Story 1.4)_.
...
| --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Node.js**                                                           | `>= 22.13` effective floor (CI pins Node 22 LTS line; dev machine v25)                                                                            | rolling                                  | high                               | ✓ CI confirmed Node 22 resolves; pnpm 11.3.0 requires Node 22.13+ (uses `node:sqlite`)                                   | Brief §5 floor is `>= 20`. **Bumped at M0 close** because pnpm 11.3.0 (also pinned) crashes on Node 20 with `ERR_UNKNOWN_BUILTIN_MODULE` for `node:sqlite`. Node 22 is current LTS — forward-aligned.  |
| **pnpm**                                                              | `>= 11.3.0` effective floor (CI uses 11.3.0; dev machine 11.3.0)                                                                                  | 2026-04 (11.3.0 line)                    | high                               | ✓ install + workspace resolve verified on 2026-05-26                                                                     | Brief §5 floor is `>= 9.15`. **Bumped at M0 close** because the `pnpm-workspace.yaml allowBuilds` mechanism (Story 1.1 spike finding) is pnpm 11+ only, and pnpm 11 transitively requires Node 22.13+. |
...
| **`cargo-deny`** _(planned M0 follow-up)_                  | latest stable                                                              | Add to CI baseline (Story 1.5) for license-policy enforcement.                                                                                                                                                                  |
