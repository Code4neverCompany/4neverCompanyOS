...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    if ((err as { code?: string }).code === "23505") {
      res
        .status(409)
        .json({ error: { code: "NAME_EXISTS", message: "Channel name already exists" } });
      return;
    }
    throw err;
  }
...
router.patch("/:id", async (req, res) => {
  ...
  if (!admin) {
    res
      .status(403)
      .json({ error: { code: "FORBIDDEN", message: "Only channel admins can update" } });
    return;
  }
...
router.delete("/:id", async (req, res) => {
  ...
  if (!admin) {
    res
      .status(403)
      .json({ error: { code: "FORBIDDEN", message: "Only channel admins can archive" } });
    return;
  }
...
router.post("/:id/join", async (req, res) => {
  ...
  await query("INSERT INTO channel_members (channel_id, user_id, role) VALUES ($1, $2, 'member')", [
    req.params.id,
    req.userId,
  ]);
