...
export function setupSocketHandlers(io: SocketIOServer): void {
  ...
  io.on("connection", (socket: AuthenticatedSocket) => {
    ...
    socket.on("message:send", async ({ channelId, content }: { channelId: string; content: string }) => {
      ...
      const user = await query("SELECT name, avatar_url FROM users WHERE id = $1", [socket.userId]);
    ...
    socket.on("message:edit", async ({ messageId, content }: { messageId: string; content: string }) => {
      const msg = await query("SELECT user_id, channel_id FROM messages WHERE id = $1", [messageId]);
      if (!msg.rows.length || msg.rows[0].user_id !== socket.userId) return;
      await query("UPDATE messages SET content = $1, updated_at = NOW() WHERE id = $2", [content, messageId]);
    ...
    socket.on("reaction:add", async ({ messageId, emoji }: { messageId: string; emoji: string }) => {
      try {
        ...
        io.emit("reaction:added", { messageId, emoji, userId: socket.userId, count: Number(countResult.rows[0].count) });
      } catch {
    ...
    socket.on("reaction:remove", async ({ messageId, emoji }: { messageId: string; emoji: string }) => {
      ...
      io.emit("reaction:removed", { messageId, emoji, userId: socket.userId, count: Number(countResult.rows[0].count) });
    });

    socket.on("presence:update", async ({ status, statusMessage }: { status: string; statusMessage?: string }) => {
      await query(
